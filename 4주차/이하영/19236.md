...
public class Main {
	...
	public static void sharkMove(Fish eat, int cnt, List<Fish> fishbowl, int[][] position) {
		...
		// 업데이트된 dir로 상어가 이동할 수 있으면 물고기들 이동 시키기
		else {
			//1번부터 물고기 이동 시작
	...
	public static void switchFish(int origin, int change, int ndir, int nr, int nc, List<Fish> fishbowl, int[][] position) {
		...
		// originFish는 넘버 그대로, dir은 ndir로, fish r, c 바꾸기
		// switchFish는 넘버, dir은 그대로, r, c 변경
...
# Fail
## 문제 해결 과정
1. 물고기 초기값 저장 및 상어 (0, 0) 배치
2. 상어 재귀 시작
   - 인자 : 먹을 물고기, 현재까지 먹은 물고기 값, 물고기 리스트, 위치 map
   1. 물고기 eat
   2. 업데이트된 방향으로 이동 가능 여부에 따라 재귀 종료 or 지속
   3. 지속한다면, 물고기 전체 이동
   4. 다음 먹이 위치 파악 및 재귀 호출
   
## 현재 상황
- 문제에 주어진 예시 테스트 케이스 4개는 전부 통과
- 오류 발생 부분과 추가 테스트 케이스 파악이 되지 않아 '틀렸습니다' 지속 발생 중
